...
class MooloolabaScraper(scraper.Scraper):
  id = "mooloolaba"
  _base_url = "https://ojm1biy2hb-dsn.algolia.net"
  _req_headers = {
        ...
        for product in result["results"][0]["hits"]:
          try:
            data["products"].append({
              ...
              "sku": str(product["sku"]),
              "url": "https://www.mooloolabamusic.com.au/" + product["url"],
              "price": float(product["price"]),
              "in_stock": product["in_stock"], # also no instock value that i can grab
            })
